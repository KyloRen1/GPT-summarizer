...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="maximum number of tokens for summarization"
    )
...
def request_summary(text_to_summarize: str, args) -> str:
    ...
    if len(text_to_summarize) * config.TOKE_WORD_COEF > config.MAX_TOKENS:
        summary = list()
        for i in trange(0, len(text_to_summarize), config.MAX_TOKENS):
            chunk = " ".join(text_to_summarize[i : i + config.MAX_TOKENS])
...
def summarize(args) -> None:
    ...
    print("Estimated number of tokens in text: ", 
          int(len(text) * config.TOKE_WORD_COEF))
